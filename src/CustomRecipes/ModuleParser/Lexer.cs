...
namespace CustomAssets.Python {
	...
	public partial class Lexer {
		...
		private readonly PythonTokens[] newLineIgnore = new PythonTokens[]
			{ PythonTokens.newline, PythonTokens.indent, PythonTokens.dedent, PythonTokens.comment };
		...
		private IExpression bitviseshift(PythonTokens[] ignore = null) {
			...
			while (IsNextOf(new PythonTokens[] { PythonTokens.shiftl, PythonTokens.shiftr }, out Token shift,
						defaultIgnore)) {
		...
		private IExpression sum(PythonTokens[] ignore = null) {
			...
			while (IsNextOf(new PythonTokens[] { PythonTokens.plus, PythonTokens.minus }, out Token shift,
						defaultIgnore)) {
		...
		private IExpression term(PythonTokens[] ignore = null) {
			...
			while (IsNextOf(
						new PythonTokens[]
							{ PythonTokens.mul, PythonTokens.div, PythonTokens.divint, PythonTokens.mod },
						out Token operToken, defaultIgnore)) {
		...
		private IExpression fstring(Token begin) {
			if (begin.value.EndsWith("\"") || begin.value.EndsWith("'")) {
				Token stringToken = new Token(begin.source, begin.lineText, begin.line, begin.column + 1,
					begin.length - 1, PythonTokens.str, begin.value.Substring(1));
				return new StringConstant(stringToken);
			}

			char endingCharacter = begin.value[1];
			// clear unrequired f on beginning
			begin = new Token(begin.source, begin.lineText, begin.line, begin.column + 1, begin.length - 1,
				PythonTokens.str, begin.value.Substring(1));
			...
			while (true) // loop trough next tokens
			{
				if (begin.value.Length > 2) {
					Token stringToken = new Token(begin.source, begin.lineText, begin.line, begin.column + 1,
							begin.length - 1, PythonTokens.str,
				...
				return new CallExpression(
						new ObjectConstant(new Constructor((args) => string.Concat(args.Values().ElementsToString()))),
		...
		private bool IsNextOf(PythonTokens[] type, out Token token, params PythonTokens[] ignore) {
			...
			if (!type.Contains(token.type)) { // revert
