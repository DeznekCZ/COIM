...
namespace PythonAPI
{
    ...
    {
        //lang=regex
        private const string fstr = @"(?<fstrbegin>f""(?:{{|[^""])*?[""{])|(?<fstrmiddle>}(?:{{|[^""])*?[""{])";
        //lang=regex
        ...
        //lang=regex
        private const string keywordsList = @"^(?:(?<none>None)|(?<btrue>True)|(?<bfalse>False)|(?<and>and)|(?<or>or)|(?<ink>in)|(?<from>from)|(?<import>import)|(?<def>def)|(?<classp>class)|(?<ifp>if)|(?<elif>elif)|(?<elsep>else)|(?<withp>with)|(?<asp>as)|(?<returnp>return)|(?<pass>pass))$";
        //lang=regex
        ...
        public static Token[] ParseFile(string file)
        {
                ...
                foreach (Match match in matchCollection)
                {
                    ...
                    foreach (Group token in match.Groups)
                    {
                        ...
                        { // search for keyword
                            ...
                            {
                                tokens.Add(new Token(fileInfo, lines[i], i + 1, token.Index + 1, token.Length, PythonTokens.name, token.Value));
                                end = token.Index + token.Length;
                            }
                            break;
                        }
