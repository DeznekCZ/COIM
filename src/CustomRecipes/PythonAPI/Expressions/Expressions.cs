...
namespace PythonAPI.Expressions {
	public class Expressions {
		...
		public static object __call__(object executable, List<(string name, object value)> arguments) {
			...
			if (executable is Method function) {
				...
				return function.Invoke(
					new NamedValue[] {
						new NamedValue("self", function.Self)
					}.Concat(named).ToArray());
			}
			if (executable is Type type) {
				Type[] types = arguments.Select(a => a.value?.GetType()).ToArray();
				ConstructorInfo constr = type.GetConstructor(types);
				if (constr is null) {
					throw new NotImplementedException("Eligible constructor was not found for: "
						+ executable.GetType().FullName + "(" + string.Join(",", types.Select(t => t.FullName)) + ")");
				}
				return constr.Invoke(arguments.Select(a => a.value).ToArray());
			}
			throw new NotImplementedException("Invocation is not defined for: " + executable.GetType().FullName);
		}

		public static object __call__async__(object executable, List<(string name, object value)> arguments) {
			if (executable is null) {
				throw new NullReferenceException("Cannot call null");
			}
		...
		public static object __range__(object left, Runtime.Range range) {
