...
{
	// T can be any IProtoWithIcon-bearing Proto. Originally constrained to
	// EntityProto for machine/vehicle pickers; broadened so EntityTypeField can
	// host non-EntityProto categories (e.g. ServerRackProto). The runtime
	// special-cases for DrivingEntityProto / ProductProto via IsAssignableTo
	// remain safe because they only fire when T actually matches.
			...
			: base()
		{
			...
			if (typeof(T).IsAssignableTo(typeof(DrivingEntityProto)))
			{
				...
				m_protoPicker = new ProtoPickerPopup<DrivingEntityProto>(
					...
					// VehicleIcon is the engine's two-in-one widget: vehicle proto +
					// fuel product overlaid in the corner.  SizeProtoLarge gives a
					// 60px icon and AbsolutePositionCenterMiddle centers it inside
					// the 64x64 picker tile (Button.IconOnly doesn't center its
					// children by default — the icon would otherwise stick to the
					// top-left).  VehicleIcon.Value with default noTooltip:false
					// sets the tooltip to "<vehicle> (<fuel>)" on the icon itself,
					// so hovering anywhere over the icon shows it.
					optionViewFactory: (DrivingEntityProto option) => {
						Button btn = new Button(Button.IconOnly);
						btn.Add(new VehicleIcon().Value(option).SizeProtoLarge().AbsolutePositionCenterMiddle());
						return btn.AsProtoPickerOptionButton();
					},
			...
			else
			{
				...
				m_protoPicker = new ProtoPickerPopup<T>(
					...
					optionViewFactory: (item) => {
						ButtonIcon btn = new ButtonIcon(Button.IconOnly, item).Size(height: 60.px());
						btn.Icon.Tooltip(item.Strings.Name);
						btn.Icon.Size(48.px()).AbsolutePositionCenterMiddle();
						return btn.AsProtoPickerOptionButton();
					},
