...
namespace ProgramableNetwork
{
	public static class MyExtensions
	{
		...
		extension(Fix32 data) {
			public ColorRgba AsColorRgba {
				get {
					byte r = (byte)(0xff & ((uint)data.RawValue >> 16));
					byte g = (byte)(0xff & ((uint)data.RawValue >> 8));
					byte b = (byte)(0xff & (uint)data.RawValue);
					return new ColorRgba(r,g,b);
				}
			}
		}

		extension(ColorRgba color) {
			public Fix32 AsFix32 {
				get {
					uint r = ((uint)color.R << 16);
					uint g = ((uint)color.G << 8);
					uint b = (uint)color.B;
					return Fix32.FromRaw(0xffffff & (int)(r | g | b));
				}
			}

			public string AsHexString => $"{color.R:X2}{color.G:X2}{color.B:X2}";
		}
	}
}
