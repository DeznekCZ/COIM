    ...
    internal class TradeOfferViewCreate : PanelRow
    {
            ...
            : base(gap: 5.px(), noBolts: true)
        {
            ...
            Body.AddAndReturn(new ButtonIcon(Button.Unity, UserInterface.Toolbar.TradeClipped_svg)
                ...
                {
                    if (supplyCount > tradeDock.Prototype.Capacity.Value)
                        return new Mafi.Core.Utils.BoolWithReason(
                            false, $"Cannot make offer bigger than capacity ({tradeDock.Prototype.Capacity})".AsLoc());

                    if (demandCount > tradeDock.Prototype.Capacity.Value)
                        return new Mafi.Core.Utils.BoolWithReason(
                            false, $"Cannot make demand bigger than capacity ({tradeDock.Prototype.Capacity})".AsLoc());

                    if (supplyCount == 0)
                        return new Mafi.Core.Utils.BoolWithReason(
                            false, $"Offer must be greater than zero".AsLoc());

                    if (demandCount == 0)
                        return new Mafi.Core.Utils.BoolWithReason(
                            false, $"Demand must be greater than zero".AsLoc());

                    return new Mafi.Core.Utils.BoolWithReason(true, $"{Tr.SellPrefix}: {supplyCount}".AsLoc());
                }, "".AsLoc())
                .OnClick(() =>
                {
