...
namespace MultiplayerContracts
{
    public class MyTabContainer : StackContainer
    {
        ...
        public void AddTab(LocStrFormatted locStr, IUiElement tabContent, out Tab tab)
        {
            ...
            m_tabs.Add(tabContent);

            tabContent.AppendTo(this);
            if (tabContent is IDynamicSizeElement dyn)
                dyn.SizeChanged += Item_SizeChanged;

            if (m_buttons.Count != 1)
                this.HideItem(tabContent);

            tab = new Tab(tabButton, tabContent);
        }
        ...
        public void SetTabVisibility(Tab tab, bool visible)
        {
            m_tabsHolder.SetItemVisibility(tab.TabButton, visible);
            if (tab.TabButton.IsOn)
            {
                for (int i = 0; i < m_buttons.Count; i++)
                {
                    if (!m_buttons[i].IsVisible())
                        continue;
                    SwitchToTab(m_tabs[i]);
                    break;
                }
            }
        }
