...
namespace MultiplayerContracts.Data {
	...
	internal class MultiplayerTradeDockInspector : BaseInspector<MultiplayerTradeDock> {
		...
		public MultiplayerTradeDockInspector(UiContext context) : base(context) {
			...
			AddPanelRow(
				...
				new ButtonIcon(UserInterface.General.Repeat_svg)
					...
					.ObserveEnabledWithReason(() => {
						if (Entity is null) {
							Status.As(MpTr.MarketNotConnected, DisplayState.Danger);
							return new Mafi.Core.Utils.BoolWithReason(false, MpTr.MarketNotConnected);
						}

						if (m_isRefeshing) {
							Status.As(MpTr.SearchingOffers, DisplayState.Warning);
							return new Mafi.Core.Utils.BoolWithReason(false, MpTr.SearchingOffers);
						}

						if (m_isNotResponding) {
							Status.As(MpTr.FailedSearchingOffers, DisplayState.Danger);
							return new Mafi.Core.Utils.BoolWithReason(true, MpTr.FailedSearchingOffers);
						}

						Status.As(Tr.EntityStatus__Working, DisplayState.Positive);
						return new Mafi.Core.Utils.BoolWithReason(true, MpTr.UpdateOffers);
					}, MpTr.UpdateOffers)
			);
			...
			m_tabs.AddTab(MpTr.OnlineOffers, m_marketList = new Column(gap: 5.px()), UserInterface.General.Tradable128_png, MpTr.OnlineOffers_tooltip, switchTo: true);
			m_tabs.AddTab(MpTr.MyOffers, m_offerList = new Column(gap: 5.px()), UserInterface.General.Sailor_svg, MpTr.MyOffers_tooltip, switchTo: false);
			m_tabs.AddTab(MpTr.Storage, m_storage = new Column(gap: 5.px()), UserInterface.EntityIcons.Storage_svg, MpTr.Storage_tooltip, switchTo: false);

			// observe offers
			this.ObserveIndexable(() => m_list.Available)
				.Do(list => {
					...
					foreach (IGrouping<string, long> offer in list.AsEnumerable()
								.GroupBy(offer => $"{m_list.Entries[offer].Demand}_{m_list.Entries[offer].Supply}")) {
						takeList.Add(new TradeOfferViewBuy(Entity, offer.ToLyst(), (item) => m_list.Entries[item],
							(item) => m_list.Available.Remove(item), context.AssetsManager, Context));
					}
				});
			...
			this.Observe(() => m_list.Claimable)
				...
				.Do((claimable, waiting) => {
					...
					m_offerList.Add(new Label(MpTr.Claimable).Width(Percent.Hundred).TextAlign(TextAlignment.CenterMiddle));
					m_offerList.Add(new HorizontalDivider());
					...
					m_offerList.Add(new HorizontalDivider());
					m_offerList.Add(new Label(MpTr.Revocable).Width(Percent.Hundred).TextAlign(TextAlignment.CenterMiddle));
					m_offerList.Add(new HorizontalDivider());
