...
namespace MultiplayerContracts
{
	public sealed class ModDefinition : DataOnlyMod {
        ...
        public ModDefinition(ModManifest manifest) : base(manifest) {
			...
			Log.Info($"{nameof(MultiplayerContracts)}: constructed");
		}


		public override void RegisterPrototypes(ProtoRegistrator registrator) {
			Log.Info($"{nameof(MultiplayerContracts)}: registering prototypes");
			...
			new MultiplayerTradeDockData().RegisterData(registrator);
